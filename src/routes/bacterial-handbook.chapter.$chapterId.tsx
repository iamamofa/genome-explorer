...
import { parts } from "./bacterial-handbook";
...
export const Route = createFileRoute("/bacterial-handbook/chapter/$chapterId")({
  component: BacterialChapter,
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: ch ? `${ch.number}. ${ch.title} — Bacterial Handbook` : "Chapter — Bacterial Handbook" },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-3xl px-6 py-24 text-center">
        ...
        <Link to="/bacterial-handbook" className="mt-4 inline-block text-primary underline">Back to handbook</Link>
      </div>
      <Footer />
    </div>
  ),
...
function BacterialChapter() {
  const { idx } = Route.useLoaderData();
  return (
    <ChapterPage
      variant="bacteria"
      handbookHref="/bacterial-handbook"
      handbookTitle="Bacterial Handbook"
